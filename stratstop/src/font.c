...
void font_draw_char(struct framebuffer *fb, uint32_t x, uint32_t y, char ch, uint8_t r, uint8_t g, uint8_t b) {
    ...
    size_t idx = (size_t)(ch - 32);
    for (uint32_t row = 0; row < STRAT_FONT_HEIGHT; row++) {
        uint8_t bits = kFont8x8[idx][row];
        for (uint32_t col = 0; col < STRAT_FONT_WIDTH; col++) {
            if (bits & (1 << col)) {
...
void font_draw_text(struct framebuffer *fb, uint32_t x, uint32_t y, const char *text, uint8_t r, uint8_t g, uint8_t b) {
    if (fb == NULL || fb->data == NULL || text == NULL) {
        return;
    }
    ...
    for (size_t i = 0; text[i] != '\0'; i++) {
        ...
        if (ch == '\n') {
            ...
            if (cursor_y + STRAT_FONT_HEIGHT > fb->height) {
                break;
            }
            continue;
        }
        ...
        if (cursor_x + STRAT_FONT_WIDTH > fb->width) {
            break;
        }

        font_draw_char(fb, cursor_x, cursor_y, ch, r, g, b);
