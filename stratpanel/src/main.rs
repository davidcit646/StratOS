...
fn draw_text(buf: &mut [u8], stride: u32, panel_width: i32, panel_height: i32,
    ...
    const FONT: [(char, [u8; 7]); 36] = [
        ...
        ('U', [0x7F, 0x40, 0x40, 0x40, 0x40, 0x00, 0x00]),
        ...
        ('D', [0x7F, 0x40, 0x40, 0x40, 0x3F, 0x00, 0x00]),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Setup: send get_registry and register id 2 as WlRegistry
    WlDisplay::new(1).get_registry(2, client.socket());
    client.registry().set_interface(2, Interface::WlRegistry);
    ...
    WlRegistry::new(2).bind(compositor_name, "wl_compositor", 4, compositor_id, client.socket());

    // Bind wl_shm
    let shm_id = client.registry().allocate();
    client.registry().set_interface(shm_id, Interface::WlShm);
    WlRegistry::new(2).bind(shm_name, "wl_shm", 1, shm_id, client.socket());
    ...
    WlRegistry::new(2).bind(layer_shell_name, "zwlr_layer_shell_v1", 4, layer_shell_id, client.socket());
    ...
    WlRegistry::new(2).bind(seat_name, "wl_seat", 7, seat_id, client.socket());
